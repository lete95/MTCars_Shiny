...
shinyServer(function(input, output) {
  ...
  model3<-lm(hp~mpg+cyl+mpgsp,data=mtcars)
  ...
  model2pred<-reactive({
      mpgInput<-input$sliderMPG
      predict(model2,newdata=data.frame(mpg=mpgInput,mpgsp=ifelse(
          mpgInput-20>0,mpgInput-20,0)))
  })
  model3pred<-reactive({
      mpgInput<-input$sliderMPG
      cyls<-input$cyls
      predict(model3,newdata=data.frame(mpg=mpgInput,mpgsp=ifelse(
          mpgInput-20>0,mpgInput-20,0),cyl=cyls)
      )
  })
  ...
  output$plot1 <- renderPlot({
    ...
    if(input$showModel3){
        model3lines<-predict(model3,newdata=data.frame(
            mpg=10:35,mpgsp=ifelse(10:35-20>0,10:35-20,0),cyl=cyls
        ))
